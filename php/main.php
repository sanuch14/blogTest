...
<div class="container">
	...
	<div class="row">
		<?php
			$data=DataBase::getArticlesDescription(0, 10);
	...
	<nav class="text-center">
		<ul class="pagination">
			<?php
				$data=DataBase::getData("SELECT COUNT(1) AS number FROM articles;");
				$row=$data->fetch(PDO::FETCH_ASSOC);
				$row=ceil((int)$row['number']/10)+1;
				for($i = 1; $i<$row; $i++){ ?>
					<li <?php if($i==1) echo 'class="active"';?>>
						<a href="/php/pageParts/pagination.php?begin=<?php echo ($i-1)*10;?>"><?php echo $i;?></a>
					</li>
			<?php
				}
			?>
